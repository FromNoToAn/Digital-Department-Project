...
## Содержание
  ...
  - [Разделы](#разделы)
    ...
    7. [API взаимодействия](#7-api-взаимодействия)
        1. [Загрузка видео](#71-загрузка-видео)
        2. [Постановка задачи](#72-постановка-задачи)
        3. [Получение информации о задаче](#73-получение-информации-о-задаче)
        4. [Остановка задачи](#74-остановка-задачи)
        5. [Управление потоком данных задачи](#75-управление-потоком-данных-задачи)
            1. [Отправка изображения для потока](#751-отправка-изображения-для-потока)
            2. [Получение информации о потоке](#752-получение-информации-о-потоке)
        6. [Управление данными задачи](#76-управление-данными-задачи)
            1. [Отправка данных задачи](#761-отправка-данных-задачи)
            2. [Получение данных задачи](#762-получение-данных-задачи)
        7. [Управление результатами задачи](#77-управление-результатами-задачи)
            1. [Отправка результатов задачи](#771-отправка-результатов-задачи)
...
- **Body:**
  - **video:** Видеофайл (опционально)
  - **is_realtime:** Флаг режима реального времени (опционально)
  - **json_data:** JSON данные (опционально)
- **Response:**
  ```json
  {
...
- **Errors:**
  - **400 - неверный формат данных**
    ```json
    {
      "detail": "Invalid JSON" или "You must provide either a video or JSON"
    }
    ```
...
#### 7.3. Получение информации о задаче
...
#### 7.5. Управление потоком данных задачи

##### 7.5.1. Отправка изображения для потока
- **Method:** POST
...
- **Response:**
  ```json
  {
    "message": "Image received for task {task_id}",
    "file_path": "videos/{task_id}.jpg"
  }
  ```

##### 7.5.2. Получение информации о потоке
- **Method:** GET
- **Endpoint:** {URL}/task/stream/{task_id}
- **Response:**
  ```json
...
#### 7.6. Управление данными задачи

##### 7.6.1. Отправка данных задачи
- **Method:** POST
...
#### 7.7. Управление результатами задачи

##### 7.7.1. Отправка результатов задачи
- **Method:** POST
- **Endpoint:** {URL}/task/results/{task_id}
- **Body:**
  ```json
  {
